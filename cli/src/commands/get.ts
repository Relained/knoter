...
export function registerGetCommand(program: Command): void {
  ...
  addGetOptions(get).action(async (target, options, cmd) => {
    await runGet([target], options, cmd);
  });

  addGetOptions(
    get
      .command("batch <targets...>")
      .description("Retrieve multiple notes")
  ).action(async (targets, options, cmd) => {
    await runGet(targets, options, cmd);
  });
}

function addGetOptions(command: Command): Command {
  return command
...
async function runGet(targets: string[], options: any, cmd: Command): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      if (targets.length === 1 && found.length === 1) {
        ...
        render(success("get batch", { found, notFound }, vaultName), format);
      }
    } finally {
      metaDb.close();
    }
    ...
    render(error(targets.length > 1 ? "get batch" : "get", code, msg), fmt);
